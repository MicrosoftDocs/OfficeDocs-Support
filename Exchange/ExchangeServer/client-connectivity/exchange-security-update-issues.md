...
### Exchange setup fails with error "Cannot start the service"

**Issue**

The Exchange cumulative update setup may fail with error "Cannot start the service Microsoft Exchange Service Host"

You may find the MS Exchange Service Host and/or all other Exchange services are stopped and in disabled mode

**Resolution** 

1) Rename the c:\ExchangeSetupLogs folder to c:\ExchangeSetupLogs.Old or some other name
2) Open Services console and change startup type of all Microsoft Exchange services to Automatic
3) Open a command prompt in administrative mode and resume the setup using following command (assuming Exchange cumulative update media mounted on D drive):
